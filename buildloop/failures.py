...
# reason unreadably long. The lookbehind leaves URLs alone: the path part of
...
def _lines(log: str) -> list[tuple[str | None, str]]:
    ...
    A leading byte-order mark would hide the first line's timestamp and drop
    it from the failing step's window.
    """
    out = []
    last_ts = None
...
def _scope(lines: list[tuple[str | None, str]], step: dict | None) -> list[str]:
...
def _failed_tests(lines: _Lines) -> _Found:
...
def _kotlin_errors(lines: _Lines) -> _Found:
...
def _gradle_block(lines: _Lines) -> _Found:
    ...
    if not block:
        return None
    headline = re.sub(r" \(registered [^)]*\)", "", block[0]).rstrip(".")
    cause = None
    for t in block[1:]:
        s = t.strip()
        if s.startswith(">"):
...
def _xcode(lines: _Lines) -> _Found:
    try:
...
def _lead_in(lines: _Lines, index: int, n: int = 8) -> list[str]:
...
def _gh_errors(lines: _Lines) -> _Found:
...
def _tool_errors(lines: _Lines) -> _Found:
...
# all-digit number is not one.
_SIG_HEX = re.compile(r"\b(?=[0-9a-f]*\d)(?=[0-9a-f]*[a-f])[0-9a-f]{7,40}\b")
